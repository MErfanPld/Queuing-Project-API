...
def send_upcoming_appointment_reminders():
    """ارسال یادآوری یک ساعت قبل از همه نوبت‌ها"""
    now = timezone.now()
    ...
    for ap in appointments:
        ...
        if now <= ap_datetime <= one_hour_later:
            ...
            phone = getattr(user, "phone", getattr(user, "phone_number", None))

            sent = send_reminder_sms(phone, name, ap_datetime)
            if sent:
